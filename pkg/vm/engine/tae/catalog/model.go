...
func init() {
	...
	SystemDBSchema, err = DefsToSchema(pkgcatalog.MO_DATABASE, pkgcatalog.MoDatabaseTableDefs)
	if err != nil {
		panic(err)
	}

	SystemTableSchema, err = DefsToSchema(pkgcatalog.MO_TABLES, pkgcatalog.MoTablesTableDefs)
	if err != nil {
		panic(err)
	}

	SystemColumnSchema, err = DefsToSchema(pkgcatalog.MO_COLUMNS, pkgcatalog.MoColumnsTableDefs)
	if err != nil {
		panic(err)
	}
