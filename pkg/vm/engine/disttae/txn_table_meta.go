...
func (tbl *txnTable) updateMeta(ctx context.Context, expr *plan.Expr) error {
	...
	if !created && !tbl.updated {
		if tbl.db.txn.engine.UsePushModelOrNot() {
			...
			if err := tbl.db.txn.engine.lazyLoad(ctx, tbl.db.databaseId, tbl.tableId, tbl); err != nil {
				panic(err)
			}
		} else {
		...
		meta, err := tbl.db.txn.getTableMeta(ctx, tbl.db.databaseId, tbl.tableId, true, columnLength, false)
		if err != nil {
			return err
		}
		tbl.meta = meta
