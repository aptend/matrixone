...
func (tbl *txnTable) isCreatedInTxn(name string) bool {
	...
	found := tbl.getTxn().engine.getLatestCatalogCache().GetTable(item)
	return !found || item.Id != tbl.tableId
}
...
func (tbl *txnTable) AlterTable(ctx context.Context, c *engine.ConstraintDef, reqs []*api.AlterTableReq) error {
	...
	// That the tableDef brought by alter requests can appended to the tail of original defs presupposes:
	...
	if !createdInTxn {
		...
		bat, err := genTableAlterTuple(reqPayload, tbl.getTxn().proc.Mp())
		if err != nil {
			return err
		}
		if _, err = tbl.getTxn().WriteBatch(ALTER, "", tbl.accountId, tbl.db.databaseId, tbl.tableId,
			tbl.db.databaseName, tbl.tableName, bat, tbl.getTxn().tnStores[0]); err != nil {
	...
	if createdInTxn {
		...
		for i, n := 0, len(txn.writes); i < n; i++ {
			if txn.writes[i].tableId == tbl.tableId {
				txn.writes[i].tableName = tbl.tableName
				txn.writes = append(txn.writes, txn.writes[i])
				txn.writes[i].note = "relocated"
...
func (tbl *txnTable) updateLogtail(ctx context.Context) (err error) {
	...
	if tbl.isCreatedInTxn(tbl.tableName) {
		return
	}

	if err = tbl.getTxn().engine.UpdateOfPush(ctx, tbl.db.databaseId, tbl.tableId,
