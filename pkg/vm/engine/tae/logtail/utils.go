...
func (data *CNCheckpointData) ReadFrom(
	...
	if version <= CheckpointVersion3 {
		...
		if bat2 != nil {
			bat2.Attrs = append(bat2.Attrs, pkgcatalog.BlockMeta_MemTruncPoint)
			bat2.Vecs = append(bat2.Vecs, committs)
		}

		bat3 := data.bats[BLKDNMetaInsertIDX]
		if bat3 != nil {
			bat3.Attrs = append(bat3.Attrs, pkgcatalog.BlockMeta_MemTruncPoint)
			bat3.Vecs = append(bat3.Vecs, committs)
		}
	}
	return
}
	...
	for _, node := range mvccNodes {
		...
		if metaNode.BaseNode.MetaLoc.IsEmpty() || metaNode.Aborted {
			if metaNode.HasDropCommitted() {
				...
				collector.data.bats[BLKDNMetaInsertIDX].GetVectorByName(pkgcatalog.BlockMeta_MemTruncPoint).Append(metaNode.Start, false)
			...
			if metaNode.HasDropCommitted() {
				...
				collector.data.bats[BLKCNMetaInsertIDX].GetVectorByName(pkgcatalog.BlockMeta_MemTruncPoint).Append(metaNode.Start, false)
				...
				collector.data.bats[BLKMetaInsertIDX].GetVectorByName(pkgcatalog.BlockMeta_MemTruncPoint).Append(metaNode.Start, false)
