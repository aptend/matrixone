...
func (entry *TableEntry) TryFindLastAppendableObject(isTombstone bool) (obj *ObjectEntry) {
	...
	ago := time.Now().Add(-5 * time.Minute).UTC().UnixNano()

	// For Appendable objects:
	// Deleting objects should be ignored, because they have been freezed, which is not valid for appending.
	dropIntent := make(map[objectio.ObjectId]struct{}, 0)
	for ok := it.Last(); ok; ok = it.Prev() {
		itObj := it.Item()
		if !itObj.IsAppendable() {
			continue
		}
		if itObj.HasDropIntent() {
			// Deleting or Deleted
			dropIntent[*itObj.ID()] = struct{}{}
			continue
		}
		if _, ok := dropIntent[*itObj.ID()]; !ok {
			// hit
			obj = itObj
			break
		}
...
func (entry *TableEntry) RecurLoop(processor Processor) (err error) {
	...
	seen := make(map[objectio.ObjectId]struct{})
	objIt := entry.MakeDataObjectIt()
	defer objIt.Release()
	for ok := objIt.Last(); ok; ok = objIt.Prev() {
		objectEntry := objIt.Item()
		if _, ok := seen[*objectEntry.ID()]; ok {
			continue
		}
		seen[*objectEntry.ID()] = struct{}{}
