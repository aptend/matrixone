...
func (cc *CatalogCache) UpdateStart(ts types.TS) {
	...
	if cc.mu.start != types.MaxTs() {
		cc.mu.start = ts
		logutil.Infof("yyyyy update catalog-cache start cache %v-%v", cc.mu.start.ToString(), cc.mu.end.ToString())
	}
}
...
func (cc *CatalogCache) ParseTablesBatchAnd(bat *batch.Batch, f func(*TableItem)) {
...
func (cc *CatalogCache) ParseColumnsBatchAnd(bat *batch.Batch, f func(map[TableItemKey]Columns)) {
...
func (cc *CatalogCache) InitTableItemWithColumns(item *TableItem, cols Columns) {
	sort.Sort(cols)
...
func (cc *CatalogCache) InsertColumns(bat *batch.Batch) {
	cc.ParseColumnsBatchAnd(bat, func(mp map[TableItemKey]Columns) {
		...
		for k, cols := range mp {
			...
			if !exist {
				logutil.Errorf("yyyyy table %v-%v-%v not found", k.AccountId, k.DatabaseId, k.Name)
				continue
			}
			cc.InitTableItemWithColumns(item, cols)
		}
	})
}

func (cc *CatalogCache) InsertDatabase(bat *batch.Batch) {
	cc.ParseDatabaseBatchAnd(bat, func(item *DatabaseItem) {
...
func (cc *CatalogCache) ParseDatabaseBatchAnd(bat *batch.Batch, f func(*DatabaseItem)) {
	rowids := vector.MustFixedCol[types.Rowid](bat.GetVector(MO_ROWID_IDX))
	timestamps := vector.MustFixedCol[types.TS](bat.GetVector(MO_TIMESTAMP_IDX))
